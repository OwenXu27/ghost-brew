...
1. **打开页面**:用 Chrome / Edge 打开 [在线地址](https://ghost-brew.vercel.app/)(Web Bluetooth 要求 HTTPS 或 `localhost`,Safari / Firefox 不支持;iPhone / iPad 上 Chrome 也不行,请用 App Store 免费的 **Bluefy** 浏览器打开)
2. **连接秤**:点左上角「连接」,在弹出的蓝牙列表里选择你的 `ECLAIR-XXXX`;秤需处于开机状态
3. **选一个配方**:内置四个预设——`CLASSIC 1:15`、`OREA`、`OREA V4`(官方 Easy Does It)、`V60`,每个都带一句「适合什么豆子」的描述;也可以稍后在配方编辑器里新建或粘贴文本导入
...
- **新建 / 编辑**:配方由若干阶段组成——注水段(到目标克重)与等待段(按秒倒计时);可设「设计粉量」(如 OREA 按 12g 设计),实际粉量不同时全部目标克数**自动等比缩放**,语音、图表、进度同步缩放;还可写一句**描述**(适合什么豆子),显示在配方列表里
- **文本快速导入**:粘贴下面这种文本直接解析成配方,首行写 `粉量: 12` 自动识别设计粉量,加一行 `说明: …` 自动成为描述:
...
- **EKG+ 水壶联动**:连接 Fellow Stagg EKG+ 手冲壶——顶部实时显示「当前水温 → 目标温度」,壶被拿起时变红;主界面 ±1° 设定目标温度、远程开关电源;冲煮开始时若水温未达标,语音/状态栏会提醒你
...
- **配方管理**:多配方(目标/编辑/复制/删除)、JSON 导入导出、文本快速导入、每配方一句「适合什么豆子」描述;内置 CLASSIC 1:15、OREA、OREA V4(官方 Easy Does It)、V60 四个预设
...
> 注:秤的手冲子页面状态(称豆/确定粉量/冲煮)与粉量**不会**通过蓝牙广播——协议里没有此类消息(已通过官方 App 逆向确认)。App 的粉量自动嗅探正是为此设计的。

## EKG+ 水壶协议摘要

Fellow Stagg EKG+ 使用 serial-over-BLE(社区逆向成果:[tlyakhov/fellow-stagg-ekg-plus](https://github.com/tlyakhov/fellow-stagg-ekg-plus)):

- Service `0x1820` / Characteristic `0x2A80`(notify + write)
- 连接后先写 20 字节握手包:`efdd 0b` + ASCII `"012345678901234"` + `9a6d`,之后壶才开始上报状态
- 帧格式:`0xEF 0xDD` + 类型字节 + 负载。类型:`0x00` 电源 / `0x01` 保温 / `0x02` 目标温度 / `0x03` 当前温度(温度 1 字节整数,后跟单位字节 `1=F`)/ `0x08` 离座状态
- 命令(8 字节):`EF DD 0A <序号> <0=电源|1=温度> <值> <序号+值> <类型>`,间隔 ≥200ms;设定温度范围 57–100°C(华氏档自动换算)
...
1. Open the page in Chrome/Edge and hit **Connect** — pick your `ECLAIR-XXXX` scale (pairs once, auto-reconnects forever). On iPhone/iPad use the free **Bluefy** browser — iOS Chrome/Safari have no Web Bluetooth
2. Pick a recipe — CLASSIC 1:15, OREA, OREA V4 (official Easy Does It) and V60 ship built in, each with a "which beans it suits" note — or paste a text recipe into quick-import
...
- **Fellow Stagg EKG+ kettle integration**: live current→target temp in the header (turns red when the kettle leaves the base), set target temp ±1° and power from the app, and a voice/footer warning at brew start if the water hasn't reached target
